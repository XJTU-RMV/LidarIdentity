...
class Referee:
    ...
    def referee_info_update(self, data, referee):
        ...
        if cmd_id == RADAR_MARK_ID:
            ...
            result = (flag & 0b1100000) >> 5
            if result==2: #默认 or 选择前哨站的时候 触发主动易伤
                self.result=1
            else:
                self.result=0
            print("云台手易伤标志:",self.result,"目标：",result)

        elif cmd_id == 0x020E: #我有几个易伤
            referee.count['time'] = data[7] & 0b11 
            self.conut = referee.count['time']
            print("易伤数量",self.conut)
            ...
            if struct.unpack('<H',data[7:9])[0]==0x0202:
                ...
                if robot_id in index_map.keys():
                    if trustable:
                        ...
                        trust_num.append(robot_id)
                    else:
                        self.sb_data_list[index_map[robot_id]][1]=999
                        self.sb_data_list[index_map[robot_id]][2]=999
            print("接收哨兵信息：",trust_num)
        elif cmd_id == 0x0001:
            try:
                stage=struct.unpack('<B',data[7:8])[0]
                if ((stage & 0b11110000) >> 4)  !=4: #赛前
                    self.conut=0
                    self.cooldown=120
                    self.send_count=0
                    print("比赛开始前，暂时不接受易伤 STAGE=",((stage & 0b11110000) >> 4))

                    self.remain_time=struct.unpack('<H',data[8:10])[0]
                    print("当前阶段剩余时间：",self.remain_time)
            except Exception as e:
                print("byd新写的代码炸了",str(e))
            ...
            try:
                ...
                for id in range(6):
                    ...
                    if x >=30 or y >= 30 or x<=0 or y <=0:
                        ...
                        y=0
                    try:
                        if int(x*100)==0 and int(y*100)==0 and id==5 and self.remain_time<=360:# 猜哨兵
                            if self.is_red:
                                x=28-6.5
                                y=7.5
                            else:
                        ...
                        print(str(e))
                    position.append([int(x*100),int(y*100)])
                    ...
                    try:
                        # ONLY BLUE!!!!!!!
                        if self.is_red:
                            if x!=0 and int(x*100)<1000 and id==0 and (flag_send==False):
                                flag_send=True
                                print("英雄来辣！！！！！")
                                self.result=1
                        else:
                            if x!=0 and int(x*100)>1800 and id==0 and (flag_send==False):
                                flag_send=True
                                print("英雄来辣！！！！！")
                                self.result=1
    ...
    def send_double(self): 
        ...
        while True:
            try:
                # print("SENDING DOUBLE...")
                if True or self.send_count>0:
                    # print(self.result)
                    ...
                    # if self.conut>0:
                    #     self.result=1
                    print("易伤发送：",self.result,self.conut,self.cooldown,self.send_count)

                    if self.result != 0 and self.conut!=0 and self.cooldown<=0: 
                        print("雷达触发易伤！！！！！！！",robot_id)
                        print("雷达触发易伤！！！！！！！",robot_id)
                        print("雷达触发易伤！！！！！！！",robot_id)
                        print("雷达触发易伤！！！！！！！",robot_id)
                        self.cooldown=60
                        ...
                        if self.send_count>= 2:
                            ...
                            self.send_count=1
                            data = struct.pack('<H', 0x0301) + struct.pack('<HHHB', 0x0121, robot_id ,0x8080 , self.send_count) #L=Bule 109 Red 9
                    else:
                    ...
                    for i in range(6): # HFF
                        ...
                        if self.is_precise[0]==1 and i==0:
                            print("TO哨兵:英雄",self.data_list[i][1],self.data_list[i][2])
                    ...
                    print("SUCCESSFULL SEND DOUBLE")
            except Exception as e:
                print(type(e),str(e))
                print("SEND DOUBLE ERROR")
                time.sleep(0.1)
                continue
    def run(self):
        """并行执行数据读取和发送"""
