...
def __callback_2(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and not param:
        param = True
def compute_reprojection_error(img,ops, pick_point, K_0, C_0, rvec, tvec):
    # 重投影物体点到图像上
    projected_points, _ = cv2.projectPoints(ops, rvec, tvec, K_0, C_0)
    for pt in projected_points:
        # print(pt)
        x,y=pt[0]
        cv2.circle(img,(int(x),int(y)),radius=10,color=(255,255,255))
    # print(ops,rvec,tvec,K_0)
...
def locate_pick(cap:Camera,color,bridge,video_test=False):
    ...
    tips = \
    {
        '0':['(0,0)','(0,28)','(15,28)','(15,0)'],
        '1':['(0,0)','(0,28)','(15,28)','(15,0)'],
    }
    ...
    print(compute_reprojection_error(info['pick_img'],ops,pick_point,K_0,C_0*0,rvec,tvec))
    cv2.imshow(info["pick_winname"], info["pick_img"])
    cv2.waitKey(1)
    cv2.waitKey(0)
    cv2.destroyWindow(info["pick_winname"])
