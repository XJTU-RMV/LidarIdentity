...
# 雷达数据部分构建示例
def build_data_radar(sendlisto):

    sendlist = []

    for p in sendlisto:
        x,y = p
        sendlist.append([np.uint16(x),np.uint16(y)])
    ...
    data = bytearray()
    data.extend(bytearray(struct.pack('H', x1)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y1)))  # 目标机器人ID (小端)

    data.extend(bytearray(struct.pack('H', x2)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y2)))  # 目标机器人ID (小端)

    data.extend(bytearray(struct.pack('H', x3)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y3)))  # 目标机器人ID (小端)

    data.extend(bytearray(struct.pack('H', x4)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y4)))  # 目标机器人ID (小端)

    data.extend(bytearray(struct.pack('H', x5)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y5)))  # 目标机器人ID (小端)

    data.extend(bytearray(struct.pack('H', x6)))  # 目标机器人ID (小端)
    data.extend(bytearray(struct.pack('H', y6)))  # 目标机器人ID (小端)
