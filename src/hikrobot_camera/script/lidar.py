...
class DepthQueue(object):
    ...
    def depth_detect_refine(self,r):
        ...
        # 采用以中心点为基准点缩小一倍的识别框，并设置ROI上界和下界，防止其超出像素平面范围
        area = self.depth[int(max(0,center[1]-r[3]/4)):int(min(center[1]+r[3]/4,self.size[1]-1)),int(max(center[0]-r[2]/4,0)):int(min(center[0]+r[2]/4,self.size[0]-1))]
...
class Radar(object):
            ...
            if Radar.__record_times > 0:
                ...
                if Radar.__record_times == 1:
                    try:
                        ...
                        with open("{0}/{1}.pkl".format(PC_STORE_DIR,datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M')),'wb') as f:
    ...
    def detect_depth(self,rects):
        ...
        results = Radar.__queue[self._no].depth_detect_refine(rects)
        Radar.__lock.release()
        return results
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 选定一个ROI区域来测深度
        cv2.imshow("img", re_frame)
        rect = cv2.selectROI("img", re_frame, False)


        _,rvec,tvec = locate_record(0,save=False) # 用四点手动标定估计位姿
        ...
        while (flag and key != ord('q') & 0xFF):
            ...
            cv2.rectangle(re_frame, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0, 255, 0), 3)
